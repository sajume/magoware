...
exports.list_chart_epg = function (req, res) {
  ...
  DBChannels.findAndCountAll({
    ...
    DBModel.findAll({
        ...
        where: {program_start: {gte: past}, program_end: {lte: future}, company_id: req.token.company_id, channel_number: {$in: channelsIds}}
      }
    ).then(function (results) {
      if (!results) {
        return res.status(404).send({
  ...
  async.auto({
    ...
    delete_epg: ['get_channels', function (results, callback) {
      if (channel_number_list.length < 1) {
        ...
        DBModel.destroy({
          where: {
            program_start: {gte: current_time},
            channel_number: {in: channel_number_list},
